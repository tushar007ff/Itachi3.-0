...
async def init():
    ...
    LOGGER("Itachi.plugins").info("𝐀𝐥𝐥 𝐅𝐞𝐚𝐭𝐮𝐫𝐞𝐬 𝐋𝐨𝐚𝐝𝐞𝐝 𝐁𝐚𝐛𝐲🥳...")
    await userbot.start()
    await Itachi.start()
    try:
        await Itachi.stream_call("https://te.legra.ph/file/29f784eb49d230ab62e9e.mp4")
    ...
    LOGGER("Itachi").info(
        "╔═════ஜ۩۞۩ஜ════╗\n  ☠︎︎𝗠𝗔𝗗𝗘 𝗕𝗬 𝗠𝗥 𝗜𝗧𝗔𝗖𝗛𝗜☠︎︎\n╚═════ஜ۩۞۩ஜ════╝"
    )
    await idle()
    await app.stop()
    await userbot.stop()
    LOGGER("Itachi").info("𝗦𝗧𝗢𝗣 𝗜𝗧𝗔𝗖𝗛𝗜 𝗠𝗨𝗦𝗜𝗖🎻 𝗕𝗢𝗧..")
